...
public class DummyPreparedStatement extends DummyStatement implements PreparedStatement {
	...
	public static PreparedStatement create(String sql, DummyConnection compondConnection) {
		DummyPreparedStatement s = new DummyPreparedStatement();
		s.sql = sql;
	...
	private PreparedStatement genTargetStatement() throws SQLException {
		return (PreparedStatement) RouterExecutionContext.call(() -> {
			PreparedStatement pstmt = this.connection.getDataSource().getTargetPreparedStmtBefExecute(
					this.connection.getDataSource(), this.sql, this.recorder.getList());
			if (pstmt == null) {
				throw new RouterException("got null prepared stmt");
			} else {
				Util.trySetConnection(pstmt, this.connection);
				this.executeResult.set(pstmt);
				list.executeWith(pstmt);
				return pstmt;
			}
		});
	}
